extension JSON {
    ...
    @frozen public struct ObjectDecoder<CodingKey>
        where CodingKey: RawRepresentable<String> & Hashable & Sendable {
        public var index: [CodingKey: JSON.Node]

        @inlinable public init(_ index: [CodingKey: JSON.Node] = [:]) {
            self.index = index
        }
    }
}
...
extension JSON.ObjectDecoder {
    @inlinable public __consuming func single() throws -> JSON.FieldDecoder<CodingKey> {
        guard let (key, value): (CodingKey, JSON.Node) = self.index.first else {
    ...
    @inlinable public subscript(key: CodingKey) -> JSON.OptionalDecoder<CodingKey> {
        .init(key: key, value: self.index[key])
    }
    @inlinable public subscript(key: CodingKey) -> JSON.FieldDecoder<CodingKey>? {

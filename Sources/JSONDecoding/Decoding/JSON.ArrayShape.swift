extension JSON {
    ...
    /// array at run time.
    @frozen public struct ArrayShape: Hashable, Sendable {
...
extension JSON.ArrayShape {
    ...
    @inlinable public func expect(count: Int) throws {
    ...
    @inlinable public func expect(multipleOf stride: Int) throws {
        guard self.count.isMultiple(of: stride) else {
            throw JSON.ArrayShapeError.init(
                invalid: self.count,
    ...
    @inlinable public func expect(that predicate: (_ count: Int) throws -> Bool) throws {
        guard try predicate(self.count) else {

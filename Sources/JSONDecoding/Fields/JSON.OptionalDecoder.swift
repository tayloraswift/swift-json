extension JSON {
    ...
    /// scenarios.
    @frozen public struct OptionalDecoder<Key> where Key: Sendable {
        public let key: Key
        public let value: JSON.Node?

        @inlinable public init(key: Key, value: JSON.Node?) {
            self.key = key
            self.value = value
        }
    }
...
extension JSON.OptionalDecoder {
    @inlinable public static func ?? (lhs: Self, rhs: @autoclosure () -> Self) -> Self {
...
extension JSON.OptionalDecoder {
    ...
    @inlinable public func decode() throws -> JSON.Node {
...
extension JSON.OptionalDecoder: JSON.TraceableDecoder {
    ...
    @inlinable public func decode<T>(with decode: (JSON.Node) throws -> T) throws -> T {

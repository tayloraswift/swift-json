extension JSON {
    ...
    public protocol TraceableDecoder {
...
extension JSON.TraceableDecoder {
    @inlinable public func decode<CodingKey, T>(
        ...
        with decode: (JSON.ObjectDecoder<CodingKey>) throws -> T
    ) throws -> T {
        try self.decode { try decode(try .init(json: $0)) }
    }
    @inlinable public func decode<T>(
        with decode: (JSON.ObjectDecoder<JSON.Key>) throws -> T
    ) throws -> T {
        try self.decode { try decode(try .init(json: $0)) }
    }
    @inlinable public func decode<T>(with decode: (JSON.Array) throws -> T) throws -> T {
        try self.decode { try decode(try .init(json: $0)) }
    }

    @inlinable public func decode<Decodable, T>(
        as _: Decodable.Type,
        with decode: (Decodable) throws -> T
    ) throws -> T where Decodable: JSONDecodable {
        try self.decode { try decode(try .init(json: $0)) }
    }
    @inlinable public func decode<Decodable>(
        to _: Decodable.Type = Decodable.self
    ) throws -> Decodable where Decodable: JSONDecodable {

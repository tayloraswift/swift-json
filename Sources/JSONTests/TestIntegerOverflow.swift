...
extension IntegerOverflow
{
    init()
    {
        self.init(integer: 256)
    }

    @Test("Integer overflow")
    func main()
    {
        // let field:JSON.FieldDecoder<Never?> = try .init(key: nil,
        //     value: try .init(parsing: self.encoded))

        // let uint8:UInt8 = try field.decode()

...
extension JSON {
    @frozen public struct ObjectEncoder<Key>: Sendable {
...
extension JSON.ObjectEncoder {
    @inlinable subscript(with key: String) -> JSON {
        _read {
            yield .init(utf8: [])
        }
        _modify {
...
extension JSON.ObjectEncoder<Any> {
    ...
    @inlinable public subscript<NestedKey>(
        key: String,
        yield: (inout JSON.ObjectEncoder<NestedKey>) -> ()
    ) -> Void {
        mutating get { yield(&self[key][as: JSON.ObjectEncoder<NestedKey>.self]) }
    }

    @inlinable public subscript(key: String, yield: (inout JSON.ArrayEncoder) -> ()) -> Void {
        mutating get { yield(&self[with: key][as: JSON.ArrayEncoder.self]) }
    }
...
extension JSON.ObjectEncoder where Key: RawRepresentable<String> {
    @inlinable public subscript(key: Key) -> JSON {
    ...
    @inlinable public subscript(key: Key, yield: (inout JSON.ArrayEncoder) -> ()) -> Void {
        mutating get { yield(&self[key][as: JSON.ArrayEncoder.self]) }
    }
